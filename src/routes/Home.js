...
class Home extends React.Component {
  state = initialData; //initialData는 지금은 따로 파일에서 하드코딩한 상태
  ...
  onDragEnd = result => {
    ...
    // 6. Task가 Column 너머 옮겨졌을 때,
    // 이제 더이상 To-do, In Progress Column에 아무 것도 없을 경우(즉 전부 Done칼럼으로 옮겨졌을 경우)
    ...
    if (!toDoIsNotEmpty && !InProgressIsNotEmpty) {
      ...
      const achievedFlower = FLOWERS[randomIndex];
      alert(`축하합니다!\n꽃이 자랐어요.\n${achievedFlower}`);
      saveFlower(achievedFlower);
      updatePlantCount()
    }
    ...
    function saveFlower(achievedFlower){
      let currentFlowers = getFlower();
      let updatedFlowers;
      if (currentFlowers) {
        updatedFlowers = currentFlowers + "," + achievedFlower;
      } else {
        updatedFlowers = achievedFlower;
      }
      localStorage.setItem(LS_KEY_FLOWERS, updatedFlowers);
    }
